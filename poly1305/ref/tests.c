...
static void print_tag(uint8_t const* nonce, uint8_t const* tag) {
  printf("Nonce:");
  for (int i = 0; i < 16; i++) {
    printf(" %02x", nonce[i]);
  }
...
int main() {
  ...
  printf("Sanity check...\n");
  {
    ...
    static char msg[] = "";
    memcpy(nonce, "\x61\xee\x09\x21\x8d\x29\xb0\xaa\xed\x7e\x15\x4a\x2c\x55\x09\xcc", 16);
    poly1305aes_athlon_authenticate(tag, (uint8_t const*) key, nonce, (uint8_t const*) msg, 0);
    if (!poly1305aes_athlon_isequal(tag, (uint8_t const*) "\xdd\x3f\xab\x22\x51\xf1\x1a\xc7\x59\xf0\x88\x71\x29\xcc\x2e\xe7")) {
      printf("Tag mismatch\n");
      print_tag(nonce, tag);
      ret = EXIT_FAILURE;
    }
...

  {
    ...
    memcpy(nonce, "\x9a\xe8\x31\xe7\x43\x97\x8d\x3a\x23\x52\x7c\x71\x28\x14\x9e\x3a", 16);
    poly1305aes_athlon_authenticate(tag, (uint8_t const*) key, nonce, (uint8_t const*) msg, 63);
    if (!poly1305aes_athlon_isequal(tag, (uint8_t const*) "\x51\x54\xad\x0d\x2c\xb2\x6e\x01\x27\x4f\xc5\x11\x48\x49\x1f\x1b")) {
      printf("Tag mismatch\n");
      print_tag(nonce, tag);
      ret = EXIT_FAILURE;
    }
  ...
  printf("Benchmark 1: single key, long message\n");
  {
    ...
    for (int i = 0; i < 4; i++) {
      ...
      poly1305aes_athlon_authenticate(tag, keybuf, nonce, buf, 1 * 1024 * 1024 * 1024);
  ...
  printf("Benchmark 2: single key, multiple short messages\n");
  {
    ...
    while (1) {
      ...
      poly1305aes_athlon_authenticate(tag, keybuf, nonce, bufs[iter % bufcount], 64);
      clock_gettime(CLOCK_MONOTONIC, &current);
  ...
  printf("\n");
  printf("Benchmark 3: single key, multiple short messages\n");
  {
    int bufcount = 1024;
    ...
    while (1) {
      ...
      poly1305aes_athlon_authenticate(tag, keybufs[iter % bufcount], nonce, bufs[iter % bufcount], 64);
      clock_gettime(CLOCK_MONOTONIC, &current);
      if (current.tv_sec > start.tv_sec && current.tv_nsec >= start.tv_nsec) {
        break;
      }
